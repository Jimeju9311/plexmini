...
<h1 align="center">PlexMini</h1>

<p align="center">A native Plex client for old 32-bit iPads and iPhones (iOS 9–10) running a jailbreak.</p>
...
The in-app control icons are drawn in code with Core Graphics rather than shipped as image assets, so they render crisply at any scale. The home-screen app icon is a PNG set generated by `assets/make_icons.py`; the PNGs are committed, so building needs no Python unless you change the artwork:

```bash
python3 assets/make_icons.py   # requires Pillow
```
...
The **logo and app icon are original work** made for this project, generated from plain geometry by `assets/make_icons.py` rather than taken from any icon set or font. They are covered by the same MIT license, so you can reuse or modify them freely.
